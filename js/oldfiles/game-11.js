...
$(document).on("vclick", ".scoresBtn", function( ev ){
	...
	//***REQUIREMENT********	
		...
		//we need to save scores on th remote server via ajax
		//we need to produce reports from the data on the remote server
	//***END REQUIREMENT********	
		console.log("EEEEEEEEEEEEEEEEEstarting to save scores here!!!");
		saveScore();
	...
	}//window loal storage check
	console.log("ajax active is "+ajaxActive);
		if(ajaxActive){
		saveScoreRemote(username,currentAnswers);}
		//if this fails, then work locally
		//we need to check for ajaxactive to prevent multiple event triggering
		
		if(ajaxActive){
			console.log("QQQQQQQQQQQQQbefore buildingajax scores pages");
			
			buildAjaxScoresPage();
			console.log("QQQQQQQQQQQQQafer building ajax code page!!!");
			}
		else
		{
			console.log("EEEEEEEEEEEEEEEEEbefore building scores pages");
			buildScoresPage();
			console.log("EEEEEEEEEEEEEEEEEafter building scores pages");
		}//ajax is not active
...
function saveScoreRemote(username,correctAnswers){
	...
	$.ajax({
		...
		data:'account='+accountName+'&username='+username+'&score='+canswersstring,
		dataType:"json",
		type:"POST"
	...
	console.log("RRRRRRRRRRRRRRRRRRRafter making remote call");
	}
}
...
function saveScore(){
	...
	if( window.localStorage ){
		...
		for (i=0;i<len;i++)
		{
			...
			console.log("RRRRRRRRRRRRRRRRuser name is "+username);
	console.log("RRRRRRRRRRRRRRRRcorrect Answers is "+correctAnswers);
				
			if(ss[i].name==username){
				console.log("RRRRRRRRRRRRRRRRuser name is equal to stored name");
				...
				for (j=0;j<5;j++){
					...
					console.log("RRRRRRRRRRRRRRRRin for loop correct answers greater than");
	
					}
					else{
						if(correctAnswers>=5){
							ss[i].scores.push(correctAnswers);
							console.log("RRRRRRRRRafter pushing scores");
							}
					
						}
						
				}
				console.log("RRRRRRRRRRRRRRRRbefore sorting");
					
				ss[i].scores.sort();
				console.log("RRRRRRRRRRRRRRRRafter sorting");
				ss[i].date=today;
				console.log("RRRRRRRRRRRRRRRRafter getting today");
				//console.log("RRRRRRRRRRRRRRRRsaving scores remote");
				//saveScoreRemote(username,correctAnswers);
				console.log("RRRRRRRRRRRRRRRRafter save score remote");
			}
			
		}

	//convert object back to string and store back in  local storage
	console.log("RRRRRRRRRRRRRRRRafter before setting in local storage");
	...
	console.log("RRRRRRRRending window local storage");
	console.log("RRRRRRRRRRRRRRRRcorrect Answers is "+correctAnswers);
				saveScoreRemote(username,correctAnswers);
...
function buildAjaxScoresPage(){
	...
	//retrieve the high scores from the remote server and build the page
	$.ajax({
		url:'get-highscore.php',
	...
	//what comes back from the server is a json object containing user names and scores
	//if( window.localStorage ){
	...
	var	html1="",html0="",html3="",min22="";;
		...
		var	min3h="<div class=\"ui-block-c headers \"><h3>LAST UPDATED DATE</h3></div>";
			...
			(scoreData).forEach(function(key,value){
			min22="";
			//Assume that scoreData has names and scores
			//assume that one user has many scores stored on thes database
			len=key.scoreData.length;
			min1="<div class=\"ui-block-a scgeneral \"><p>"+key.name+"</p></div>";
			for (i=0;i<len;i++){
				if(key.scores[i]>0){
				min22=min22+"<p>"+key.scores[i]+"</p>";
