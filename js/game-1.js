...
$(document).on("click", ".playBtn", function( ev ){
	...
	$("#scoretable").children().remove(".bclass");
	...
	if( !scriptLoaded ){
		...
		var p=
		"http://m.edumedia.ca/dani0004/mad9014/quizpart2/data/capital-json.js?&callback=buildQuestions";
...
$(document).on("vclick", ".scoresBtn", function( ev ){
	...
	//***END REQUIREMENT********	
		...
		scoreSaved=localStorage.getItem("scoresaved");
		var len=username.length;
		//user may choose to bail out and click scores button before they 
		//finish the game
		//or they may have completed and want to see the score
		//if they have completed, the score is already saved
		
		if(len>0){
		
			saveScoreRemote(username,correctAnswers);
		}
...
$(document).on("vclick", ".replay", function( ev ){
	
	//or just drop the div and build again - cleaner
	//remove what is in the game area
	//manually trigger the click on th eplay button
	
	scriptLoaded=false;
	//$("#answerList").remove();
	//$("#gamearea").remove("#questions");
	//$("#gamearea").clear;
	$("#scoretable").children().remove(".ui-grid-b");
	$("#scoretable").children().remove(".bclass");
	$("#help").children().remove("#helpList");
	...
	//remove the json script so that it gets reloaded
	$("#dataObject").remove();
	
	
	//add the new script tag via jquery and reload the script so that the get //runs
	//if(!scriptLoaded){
	var scriptsrc="http://m.edumedia.ca/dani0004/mad9014/quizpart2/data/capital-json.js?&callback=buildQuestions";
	
	
		var mm="<script src=\""+scriptsrc+"id=\"dataObject\">";
		$(mm).appendTo("head").trigger("create");
		
		scriptLoaded=true;
	//}
	
	
	$.mobile.changePage("#play","flip",true,false);
	$("#play").fadeIn('slow');
	$("#scores").hide();
	$("#welcome").hide();
});///end testing play again button
...
function buildQuestions( data ){
	...
	//$("#gamearea").clear;
	//$("#gamearea").children().remove("#questions");
	$("#gamearea").remove("#questions");
	//$("#questions").remove();
...
$(document).on("click", ".questionsList li", function( ev ){
	...
	//	saveScoreRemote(username,correctAnswersString);}
...
function saveScoreRemote(username,correctAnswers){
	...
	var accountName="dani0004";
	   ...
	   scoreString ='account=dani0004&username='+username+'&score='+correctAnswers;
        $.ajax({
    url:'http://m.edumedia.ca/dani0004/mad9014/quizpart2/data/set-highscore.php',
    data: scoreString,
    dataType:"json",
    type:"POST"
}).done(function(scoreData, textStatus, jqXHR){
    //this is the code and message about the success or failure to save a score
   //jump to the scores page to show the user the current scores
   scoreSaved="0";
	if(window.localStorage){localStorage.setItem("scoresaved","0");}
   //$.mobile.changePage("#scores");
	...
	showErrorMessage("1",errorThrown);
});
...
/* this function now deprecated replaced by saveScoreRemote*/
function saveScore(){
...
/* this function replaces the deprecated build scores **/
function buildAjaxScoresPage(){
	
	//extract the data from the server
	//objects are score and username

	$.ajax({
	url:"http://m.edumedia.ca/dani0004/mad9014/quizpart2/data/get-highscore.php",	
    data:'account=dani0004',
    dataType:"json",
    type:"POST"
}).done(function(scoreData, textStatus, jqXHR){
    //got the list of scores
	...
	showErrorMessage("2",errorThrown);
});
...
function showErrorMessage(id,msg){
	//attach to msg area in the main div
	var minihtml="",html="",minimsg="";
	
	switch(id){
		case "1":
		minimsg="Unable to save scores";
		break;
		case "2":
		minimsg="Unable to retrieve scores";
		break;
		default:
		minimsg="General error with external server";
	}
	
	minihtml=minihtml+"<div   data-theme=\"b\"  ><h4>"+minimsg+"</h4><p> "+msg+"</<p ></div>";
	
		
	html="<div id=\"errorMsg\" data-role=\"popup\" class=\"ui-content\" data-position-to=\"window\" data-theme=\"a\"><a href=\"#\" data-rel=\"back\" data-role=\"button\" data-theme=\"a\" data-icon=\"delete\" data-iconpos=\"notext\" class=\"ui-btn-right\">Close</a><h3><a name=\"errorMsg\">Check Answers<a></h3>"+minihtml+"</div>";
...
function formatReport(scoreData){
	
	
	var ss=scoreData.scores;
	var len=ss.length;
	
	//we need to sort this array by names
	
	var ss2= sortByKey(ss,"username");

	
	var	min1="",row1="",html1="",min22="",min23="",min24="",len=0;
	
		var pp1="0",pp2="0",pp3="0",html="",minihtml="",min1="",min2="",min3="";
		min24="<div class=\"bclass\" data-theme=\"a\"> <a href=\"#welcome\" data-role=\"button\"  id=\"playagain\" class=\"replay\" >Play Again</a></div>";
		var	min1h="<div class=\"ui-block-a headers \"><h3>PLAYER NAMES</h3></div>";
		var	min2h="<div class=\"ui-block-b headers \"><h3>SCORES</h3></div>";
		
		var	html0="<div class=\"ui-grid-b outer\" >"+min1h+min2h+"</div>";
			
			min22="",username1="";
			ss2.forEach(function(key,value){
			if(!(key.username==username1)){
				username1=key.username;
				min23="<div class=\"ui-block-b scgeneral2 \">"+min22+"</div>";
				min1="<div class=\"ui-block-a scgeneral \"><p>"+key.username+"</p></div>";
				row1=row1+(min1+min23);
	...
	len=row1.length;
	if(len>0){
				html1=html1+"<div class=\"ui-grid-b outer\" >"+row1+"</div>";
				}
		});
		
		var html3=min24+html0+html1;
...
function buildHelpPage(){
	...
	$("#help").children().remove("#helpList");
			...
			$(minihtml2).appendTo("#helpdata ").trigger("create",function(ev){
		ev.stopPropagation();
