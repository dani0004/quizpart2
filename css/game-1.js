...
$(document).on("vclick", ".helpBtn", function( ev ){
	...
	//### EVENT FIRING CONTROL SECTION
	console.log("building help page");
	$("#help div:last-child").children().remove("#helpList");
	ev.stopPropagation();
	console.log("building help page2");
	var mm=localStorage.getItem("helppage");
	console.log("building help page3");
	console.log(mm);
		if(mm=="1"){return;}
	//###END EVENT FIRING CONTROL SECTION
			...
			//if( window.localStorage ){
				...
				if(s!=null ){
					...
					if(!userexists){
						...
						json["date"]=JSON.stringify(today);
						console.log(ss);
						ss.push(json);
...
$(document).on("click", ".questionsList li", function( ev ){
	...
	console.log("WWWWWWWWWWWWWchecking twicetot");
	...
	console.log("WWWWWWWWWWWWWENDchecking twicetot");
	//update scores
...
function buildHelpPage2(){
		...
		console.log("before inserting help");
		var mm=localStorage.getItem("helppage");
		if(mm=="1"){return;}
		console.log("mm is "+mm);
		$("#helpList").remove();
...
//set an event to close the help diaog box
